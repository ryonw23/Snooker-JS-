...
Menu:
The menu displays all available game modes and options.
The game modes include those described in task 3.
The menu also explains placing the cue ball in the D zone. 
Options allow toggling features such as the menu itself, aim assistance, weather conditions (extension), and control preferences.
Instructions for operating the mouse or key controls are provided when switching between the two. 

Modes:
When pressing a key, the app checks whether it is 1, 2 or 3 and assigns the mode to that number. 
Mode 1 sets up all balls at the standard positions.
Mode 2 sets up the colour balls at the standard positions, and randomises the positions of the red balls.
Mode 3 randomises the positions of all balls within the boundary of the table.
All randomisations are restricted within the boundary of the table.

Options:
Menu toggles the menu display on or off.
Aim assistance would provide users with a guiding line for striking.
Weather cycles through the weather conditions one at a time.
Toggling the controls would switch between mouse and key controls.

Controls:
As some users prefer different ways to play, both mouse and key controls are implemented, and aim assistance is available as a toggle.

Mouse controls work by rotating the cue stick around the ball, following the mouse.
The app checks if a mouse is pressed near the cue stick and users can drag the cue stick away from and towards the cue ball.
Users strike the cue ball upon releasing the mouse click.

The key controls work by rotating the cue stick with user control using the left and right arrow keys.
The distance of the cue stick is controlled by the up and down arrows.
Using spacebar strikes the cue ball.

The strength of the strike increases as the cue stick moves further from the ball.

Weather conditions extension:
Snooker tables are typically unaffected by weather.
Implementing this feature would provide a unique gameplay element.
Users can toggle between the weathers for either traditional gameplay or challenging gameplay.
The weather changes the scene of the table, affecting the table colours as well as showing snowfall and rainfall.

The initial state of the game is standard weather with normal behaviour for the table and balls.

The first weather condition is snow. This "freezes" the table and balls, making the table slippery, and the balls and walls hard.
The friction of the balls are reduced, the restitution of the balls and walls are increased, and the striking power of the cue is increased.
The balls would slide more across the table, and bounce more against each other and the walls.

The second weather condition is rain. This introduces a bogged-down table, making the table less slippery, and the balls and walls soft.
The friction of the balls are increased, the restitution of the balls and walls are decreased, and the striking power of the cue is decreased.
The balls move slower, and bounce less against each other and the walls.

*/

// (498 words including headers)